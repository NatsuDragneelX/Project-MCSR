  ...
  return new Promise((resolve, reject) => {
    ...
    function noPathListener(results) {
      if (results.path.length === 0) {
          ...
          cleanup(error('NoPath', 'No path to the goal! Retrying...'));
          bot.chat("No path to the goal. Retrying...");
          retryPath(); // Retry the pathfinding with fallback
      } else if (results.status === 'timeout') {
          cleanup(error('Timeout', 'Took too long to decide path to goal! Retrying...'));
          bot.chat("Pathfinding timed out. Trying another approach...");
          retryPath();
      }
  }  
